...
const LanguagePage = () => {
  ...
  const [noteEntity, setNoteEntity] = useState(useSelector((state) => state.languages.currentNote));
  ...
  return (
    ...
    >
      ...
      >
        ...
        {noteEntity?.name && <div style={{ marginRight: "5px", fontSize: "38px" }}>{toTitleCase(noteEntity?.name)}</div>}
      ...
      >
        {!noteEntity.notes || noteEntity?.notes.length === 0 ? (
          ...
          [...noteEntity?.notes].reverse().map((note, index) => (
            <Note
              key={index}
              id={note._id}
              title={note.title}
