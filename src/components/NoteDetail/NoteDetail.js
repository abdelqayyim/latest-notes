...
import { updateNote, setCurrentNote, addText, setSpinnerMessage }  from '../../redux/slice';
...
const NoteDetail = ({ note, setNote }) => {
  ...
  const handleAddCode = () => {
    ...
    const updatedNoteDetail = note?.noteDetail ? [...note.noteDetail, { type: "text", content: "Write code here", language:"javascript" }] : [{ type: "text", content: "", language:"javascript" }];
  ...
  const handleSaveNote = async () => {
    try{
      ...
      let newNoteDetail = [];
      refs.current.forEach((ref) => { // send the updated noteDetail in the request
        newNoteDetail.push(ref.current);
      });
      const response = await LanguageServices.updateNote({language_id: selectedLanguageID, title: note.title, description: note.description, note_detail:newNoteDetail, note_id: note._id});
  ...
  const moveElement = (index) => {
    if (index <= 0 || !note?.noteDetail || index >= note.noteDetail.length) return; // Prevent invalid moves
  
    let tempNoteDetail = [...note.noteDetail];
    let tempRefs = [...refs.current];
  
    // Swap the current element with the previous one
    [tempNoteDetail[index - 1], tempNoteDetail[index]] = [tempNoteDetail[index], tempNoteDetail[index - 1]];
    [tempRefs[index - 1], tempRefs[index]] = [tempRefs[index], tempRefs[index - 1]];
...
const handleAddImage = async (event) => {
  ...
  const actions = [
    ...
    { icon: <ImageIcon />, name: "Add Image", action: () => fileInputRef.current.click()},
    { icon: <SaveOutlinedIcon />, name: "Save Note", action: () => handleSaveNote()},
  ];

  useEffect(() => {
    if (note?.noteDetail) { // this will only go through for new elements added
  ...
  return (
    <div style={{marginTop:"10px",  flexGrow:"1", overflowY:"visible", paddingBottom: "20px"}}>
      {note?.noteDetail?.map((info, index) => {
        if (info.type === "text") {
          return <Text ref={refs} removeElement={removeElement} detail={info} key={info._id || `temp-${Math.random()}`} index={index} moveElement={ moveElement}/>;
        }
        if (info.type === "img") {
          return (
            <IMG img={info.content} key={index} index={index} removeElement={removeElement} moveElement={ moveElement} />
          );
        }
      })}
      <input
