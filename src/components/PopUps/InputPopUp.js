...
const AddLanguagePopUp = (props) => {
  ...
  let currentLanguageID = useSelector(
  ...
  const languageExists = (val) => {
    // checks to see if the language currently exists or not
    let value = null;
  ...
  const addNote = async () => {
    ...
    try {
      ...
      dispatch(
        setErrorMessage({ message: "Note sucessfully added", sign: "positive" })
      );
